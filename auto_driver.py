...
async def recv(client, message):
    if os.path.exists("temp_time.json"):
        ...
        if message.chat.id != data["user"]:
            return
        if len(data) == 1:
            date = re.search(r"[0-3]{1}\d{1}-[0-1]{1}\d{1}-\d{4}", message.text)
            if date is not None:
            ...
            p = (
                ("{0}|{1:02}-{2:02}".format(data["date"], int(start[0]), int(start[1]))),
                ("{0}|{1:02}-{2:02}".format(data["date"], int(end[0]), int(end[1])))
                )
            data["start"] = p[0]
            data["end"] = p[1]
...
async def json_writer(exec_url, session_id):
    with open("session_handler.json", "w") as writer:
        ...
        data["exec_url"] = exec_url
        LOGGER.debug("session_handler created with date : {Date}  id: {session_id} url: {exec_url}".format_map(data))
...
async def auto_join(meeting_link, message):
    ...
    try:
        ...
        signin = browser.find_element_by_css_selector(
            ".style-rest-1IrDU.style-theme-primary-2Khdp.style-size-large-1-1tY.style-botton-outline-none-1M0ur")
        signin.click()
        await message.edit("Now Working, Progress 80%")
        join_meeting = WebDriverWait(browser, time_to_wait).until(EC.element_to_be_clickable(
            (By.CSS_SELECTOR,
        ...
        try:
            ...
            await message.reply_text(
                "<b>automatic mic disabling failed please make sure you are in the \
right page using <code>/screenshot</code> command and use <code>/mute</code> command</b>")
    except Exception:
        pass
...
async def toolkit(client, message):
    try:
        LOGGER.debug(f"reading session_handler for possible datas, initiated for {message.command}")
        _, session_id, exec_url = await json_reader()
        LOGGER.debug(f"trying to connect session for with id : {session_id} url: {exec_url}, initiated for {message.command}")
        browser = attach_to_session(exec_url, session_id)
        LOGGER.debug(f"successfully attached to session, initated for {message.command}")
        await asyncio.sleep(1)
        if message.command[0] == "screenshot":
            # https://github.com/SpEcHiDe/UniBorg/blob/cfbb5bb97dafe7d87ce3772279583160954d312c/stdplugins/screencapture.py#L41
            im_png = browser.get_screenshot_as_png()
            with io.BytesIO(im_png) as out:
                out.name = "@Cisco.ScreenCapture.PNG"
                await client.send_photo(photo=out, chat_id=message.chat.id)
...
async def callback_handler(client, callback_query):
    ...
    # below code read the timetable.json file and list every date :)
        ...
        if len(data) == 0:
            ...
            return
        list_comp = [
            [InlineKeyboardButton(text=x, callback_data=y)] for x, y in
            [("Exit", "exit") if x >= len(data) else (data[x][2], f"fetch={x}") for x in range(len(data) + 1)]]
    ...
    # fetches lecture list with the given key, in this case date
        ...
        markup = [
            [InlineKeyboardButton(text=x, callback_data=y)] for x, y in
            [("Exit", "exit") if x >= len(data) else (data[x][0], f"data={x}={db_name}") for x in range(len(data) + 1)]]
...
async def send_message(_, message):
    ...
    # Lock file is created for this instance
        ...
        try:
            ...
            await asyncio.sleep(1)
            # Webdrive Stuffs :)
            chat_all = WebDriverWait(browser, time_to_wait).until(
                EC.element_to_be_clickable((By.CSS_SELECTOR, ".menu-item.icon-chat.item-chat.btn-52")))
            chat_all.click()
            text_space = WebDriverWait(browser, time_to_wait).until(
                EC.element_to_be_clickable((By.XPATH, "//textarea[@title='Enter your message for everyone']")))
